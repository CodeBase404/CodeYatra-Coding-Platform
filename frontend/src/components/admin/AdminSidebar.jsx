...
function AdminSidebar({ onToggleCollapse, isMobile, toggleMobileSidebar }) {
  ...
  const menuItems = [
    ...
    { id: "videos", label: "Videos Solutions", icon: Video },
    { id: "contest", label: "Contests", icon: Trophy },
    { id: "profile", label: "Profile", icon: User },
  ];
  return (
    <div
      className={`bg-white dark:bg-neutral/10 border-r border-gray-200 dark:border-white/10 h-[94%] flex flex-col shadow-sm transition-all duration-300 ${
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 md:w-9 h-8 md:h-8 overflow-hidden bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl flex items-center justify-center">
            <img src={user?.profileImage?.secureUrl} className="w-4 md:w-8 h-4 md:h-8 text-white" />
          </div>
          {(!isCollapsed || isMobile) && (
            <div className="min-w-0">
              ...
              <p className="text-[10px] md:text-xs text-gray-500 dark:text-white/80 truncate">
                {user?.emailId}
              </p>
            </div>
          )}
